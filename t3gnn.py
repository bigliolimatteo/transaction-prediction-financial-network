        ...
        for z in range(self.num_gnn_layers):
            ...
            if isnap > 0:
                if self.update=='gru' or self.update=='linrnn':
                    if self.update=='gru': h = torch.Tensor(self.updates[z](h, self.previous_embeddings[z].clone()).detach().numpy())
                    if self.update=='linrnn': h = torch.Tensor(self.updates[z](h, self.previous_embeddings[z].clone())[0].detach().numpy())
